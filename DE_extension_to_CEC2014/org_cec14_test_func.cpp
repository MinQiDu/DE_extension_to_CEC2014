...
/* 外部會呼叫的兩個函式 1.cec14_test_func() 2.GetBounds() */
// 外部輸入的資訊:
// x = 欲評估的解向量（維度為 nx）
// f = 存放評估結果的位址（可裝多個 fitness）
// nx = 問題維度（例如 10、30、50）
// mx = 一次評估幾筆解（通常是 1）
// func_num = 要執行的 benchmark function 編號（1 ~ 30）
void cec14_test_func(double* x, double* f, int nx, int mx, int func_num)
{
	int cf_num = 10, i, j; // 在資料準備階段統一配置 cf_num = 10 的 buffer，實際用幾個在每個 function 再說。
	if (ini_flag == 1)
	{
	...
	/* 讀入評估函數運算所需 data (1. matrix 2. shift_data 3. shuffle_data)*/
	if (ini_flag == 0)
	{
		FILE* fpt;
		char FileName[256];
