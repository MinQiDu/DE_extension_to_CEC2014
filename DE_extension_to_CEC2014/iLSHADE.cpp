...
#include "iLSHADE_fileoutput.h" /* 包含 FileOutput 函式的宣告 */

/* Constructor 初始化亂數引擎 */
algo_iLSHADE::algo_iLSHADE()
	: gen(rd()) /* 用 rd 初始化 gen（記住這只能放在初始化列表中） */
{
}

void algo_iLSHADE::RunALG(int _dim, int _pop_size, double _mCR, double _mF, double _H, double _p, int _func_id, bool _archive_flag)
{
	int run = 51; // 設定運行次數
...
void algo_iLSHADE::Init()
{
	...
	MCR.assign(H, 0.8); // 初始化 MCR = 0.8 (iL-SHADE策略)
...
void algo_iLSHADE::Mutation()
{
	...
	for (int i = 0; i < pop_size; ++i)
	{
		...
		/* iL-SHADE策略 : MCR & MF 某一個 entry 固定為0.9 */
		if (r == H - 1) {
			MCR[r] = 0.9;
			MF[r] = 0.9;
		}
		...
		/* iL-SHADE策略 : 階段性控制部出現過低CR[i] -> 探索性太低 */
		if (nfes < 0.25 * mnfes) CR[i] = max(CR[i], 0.5);
		else if (nfes < 0.5 * mnfes) CR[i] = max(CR[i], 0.25);

		do {
			F[i] = dist_F(gen);
		} while (F[i] <= 0.0 || F[i] > 1.0);       /* truncate */
		/* iL-SHADE策略 : 階段性控制部出現過高F[i] -> 步長倍率太高導致初期不穩 */
		if (nfes < 0.25 * mnfes) F[i] = min(F[i], 0.7);
		else if (nfes < 0.5 * mnfes) F[i] = min(F[i], 0.8);
		else if (nfes < 0.75 * mnfes) F[i] = min(F[i], 0.9);
...
void algo_iLSHADE::Determination()
{
	sF.reserve(pop_size);
...
void algo_iLSHADE::ParaAdaptation()
{
	...
	/* iL-SHADE 策略 : 新一代加權值 + 上一代原始值 */
	MCR[k] = (mean_sCR + MCR[k]) / 2; // Update MCR for the current generation
	...
	/* iL-SHADE 策略 : 新一代加權值 + 上一代原始值 */
	MF[k] = (mean_sF + MF[k]) / 2; // Update MF for the current generation
	...
	/* iL-SHADE策略 : 更新 p 漸漸從0.2 -> 0.1 */
	double p_max = 0.2;
	double p_min = 0.1;
	p = p_max - ((p_max - p_min) / static_cast<double>(mnfes)) * static_cast<double>(nfes);
...
void algo_iLSHADE::LPSR()
{
	...
	if (next_pop_size < Nmin) {
		next_pop_size = Nmin; // 確保 next_pop_size 不小於 Nmin
	}
	else if (next_pop_size < pop_size) {
		// 1. 對當前人口依照 fitness 由小到大排序
		vector<pair<double, vector<double>>> ranked_fit_pop(pop_size);
		for (int i = 0; i < pop_size; ++i) {
			ranked_fit_pop[i] = { fit[i], pop[i] };
		}
		sort(ranked_fit_pop.begin(), ranked_fit_pop.end(),
			[](const pair<double, vector<double>>& a, const pair<double, vector<double>>& b) {
				return a.first < b.first; // 按 fitness 排序, fitness 較小的在前面（越好）
			});

		// 2. 對當前人口刪除最差的 pop_size - next_pop_size 個解
		pop.clear();
		fit.clear();
		for (int i = 0; i < next_pop_size; ++i) {
			pop.push_back(ranked_fit_pop[i].second); // 保留最好的解
			fit.push_back(ranked_fit_pop[i].first);  // 保留對應的 fitness
		}

		// 3. 重新調整 A 大小要等於 next_A_size
		int next_A_size = static_cast<int>(r_archive * next_pop_size); // 計算新的 A 大小
		if (archive_flag) {
			while (A.size() > next_A_size) {
				uniform_int_distribution<int> dist_idx(0, A.size() - 1);
				int idx = dist_idx(gen);
				A.erase(A.begin() + idx); // 刪除隨機一個元素
			}
			while (A.size() < next_A_size) {
				A.push_back(vector<double>(dim, 0.0)); // 填充空解
			}
		}
